...
class Group:
    ...
    def __str__(self) -> str:
        ...
        for names in self._members:
            student_names = student_names + names.name + ' , '
        return f'{0}'.format(student_names)
...
class Grouping:
    ...
    def __str__(self) -> str:
        ...
        for group in self._groups:
            ...
            member_names += '\n'
        return f'{0}'.format(member_names)
    ...
    def add_group(self, group: Group) -> bool:
        ...
        """
        member_ids = []
        # No group in _groups contains zero member
        if len(group) == 0:
            return False
        else:
            # remove non-unique students
            set_members = set(group.get_members())
            # Check if student appears in more than one group in _groups
            # First append all student ids
            for member in group.get_members():
                member_ids.append(member)
                # if student appears in more than one group (ie is not
                # unique) then return False
            if len(set_members) != len(member_ids):
                return False
...
class AlphaGrouper(Grouper):
    ...
    def make_grouping(self, course: Course, survey: Survey) -> Grouping:
        ...
        """
        student_list = sort_students(course.students.copy(), 'name')
        student_list = slice_list(student_list, self.group_size)
        final_grouping = Grouping()

        for student in student_list:
            group = Group(student)
            final_grouping.add_group(group)
    ...
    def make_grouping(self, course: Course, survey: Survey) -> Grouping:
        ...
        for student in student_tuple:
            if student in student_list:
                temp_group = [student_list[0]]
                student_list.pop(0)
                while self.group_size > len(temp_group) and len(
                        student_list) > 0:
                    best_student = find_best_addition_to_group(
                        survey, temp_group, student_list)
                    temp_group.append(best_student)
                    student_list.remove(best_student)
                final_grouping.add_group(Group(temp_group))
        return final_grouping
...
class SimulatedAnnealingGrouper(Grouper):
    ...
    def make_grouping(self, course: Course, survey: Survey) -> Grouping:
        ...
        initial_grouping = slice_list(students, self.group_size)
        initial_score = total_score(survey, initial_grouping)
        temp = self.initial_temperature
        final_grouping = Grouping()

        i = 0
        while i < self.iterations - 1:
            new_grouping = deepcopy(initial_grouping)
            random_swap(new_grouping, i)
            score = total_score(survey, new_grouping)
            temperature = (temp * (1 - (i / (self.iterations - 1))))
            result = accept(initial_score, score, temperature, i)
            if result is True:
                initial_score = score
                initial_grouping = new_grouping
                i += 1
            else:
                i += 1
                continue
        for item in initial_grouping:
            final_grouping.add_group(Group(item))
        return final_grouping
